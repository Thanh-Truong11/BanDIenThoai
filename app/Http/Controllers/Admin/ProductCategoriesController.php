...
class ProductCategoriesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Tạo slug tự động nếu không có trong form
        $slug = $request->input('slug') ?? Str::slug($validatedData['category_name']);
    
        // Kiểm tra tính duy nhất của slug
        if (ProductCategory::where('slug', $slug)->exists()) {
            return redirect()->back()->withErrors(['slug' => 'Slug đã tồn tại.'])->withInput();
        }
    
        // Lưu productCategories mới vào cơ sở dữ liệu
        $productCategory = new ProductCategory();
        $productCategory->category_name = $validatedData['category_name'];
        $productCategory->slug = $slug; // Lưu slug
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Kiểm tra nếu tên danh mục đã thay đổi và slug phải duy nhất
        if ($productCategory->category_name !== $validatedData['category_name']) {
            if (ProductCategory::where('slug', $slug)->where('id', '!=', $id)->exists()) {
                return redirect()->back()->withErrors(['slug' => 'Slug đã tồn tại.'])->withInput();
            }
        }
        ...
        $productCategory->category_name = $validatedData['category_name'];
        $productCategory->slug = $slug; // Update slug
        $productCategory->status = $validatedData['status'];
        $productCategory->save();
